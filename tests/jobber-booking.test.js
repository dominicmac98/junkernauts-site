...
function setup({ booking = true, connection = {}, lastWarm, storageBlocked = false } = {}) {
  const frames = [];
  const events = {};
  const timers = new Map();
  const storage = new Map(lastWarm ? [["junkernauts-jobber-warmed", String(lastWarm)]] : []);
  let timerId = 0;
  const status = { hidden: false, textContent: "Connecting to booking..." };
  const link = {};
  const mount = { appendChild: f => frames.push(f), setAttribute: (k, v) => { mount[k] = v; }, scrollIntoView: () => { mount.scrolled = true; } };
...
test("background warmup waits for page load and idle, and never duplicates the frame", () => {
  ...
  t.message("887px");
  assert.ok(t.storage.has("junkernauts-jobber-warmed"));
